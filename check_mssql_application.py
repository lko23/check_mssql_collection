...
MODES = {
    ...
    'license'       : { 'help'      : 'Number of License',
...
def return_nagios(options, stdout='', result='', unit='', label='', perfdata=''):
    if result is None:
        prefix = 'UNKNOWN: '
        code = 3
    ...
    strperfdata = str(perfdata)
    try:
        stdout = stdout % (strresult)
    except TypeError as e:
        pass
    if len(strperfdata) > 0:
        stdout = '%s%s|%s' % (prefix, stdout, strperfdata)
    else:
...
class MSSQLQuery(object):
    ...
    def __init__(self, query, options, label='', unit='', stdout='', perfdata='', host='', modifier=1, *args, **kwargs):
        self.query = query
        self.label = label
        self.unit = unit
        self.stdout = stdout
    ...
    def calculate_result(self):
        # extract perfdata if part of query
        if isinstance(self.query_result, str) and "|" in self.query_result:
            self.query_result_arr = self.query_result.split("|",1)
            self.query_result = self.query_result_arr[0]
            self.query_perfdata = self.query_result_arr[1]
            self.perfdata = self.query_perfdata
...
# remove pickle and write time and value to simple to line file
    ...
    def make_name(self):
        ...
        tmpdir = '/var/tmp'
        self.name = '%s/check_mssql_server-%s-%s.tmp' % (tmpdir, self.host, self.options.mode)
